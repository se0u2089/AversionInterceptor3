...
AAPlayerCameraManager::AAPlayerCameraManager(const class FObjectInitializer& ObjectInitializer)
...
void AAPlayerCameraManager::BeginPlay()
{
	Super::BeginPlay();
...
void AAPlayerCameraManager::UpdateCamera(float DeltaTime)
{
	AInterceptorCharacter* MyPawn = PCOwner ? Cast<AInterceptorCharacter>(PCOwner->GetPawn()) : nullptr;
	if (MyPawn)
	{
