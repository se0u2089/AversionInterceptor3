...
ADroneBot::ADroneBot(const class FObjectInitializer& ObjectInitializer)
...
void ADroneBot::BeginPlay()
{
	...
	if (PawnSensingComp)
	{
		PawnSensingComp->OnSeePawn.AddDynamic(this, &ADroneBot::OnSeePlayer);
		PawnSensingComp->OnHearNoise.AddDynamic(this, &ADroneBot::OnHearNoise);
	}
	if (MeleeCollisionComp)
	{
		MeleeCollisionComp->OnComponentBeginOverlap.AddDynamic(this, &ADroneBot::OnMeleeCompBeginOverlap);
	}
...
void ADroneBot::Tick(float DeltaSeconds)
{
	...
	{
		ADroneAIController* AIController = Cast<ADroneAIController>(GetController());
		if (AIController)
		{
			bSensedTarget = false;
			/* Reset */
...
void ADroneBot::OnSeePlayer(APawn* Pawn)
{
	if (!IsAlive())
	{
		return;
	...
	ADroneAIController* AIController = Cast<ADroneAIController>(GetController());
	AAversionInterceptorCharacter* SensedPawn = Cast<AAversionInterceptorCharacter>(Pawn);
...
void ADroneBot::OnHearNoise(APawn* PawnInstigator, const FVector& Location, float Volume)
{
	if (!IsAlive())
	{
		return;
...
void ADroneBot::PerformMeleeStrike(AActor* HitActor)
{
...
void ADroneBot::SetBotType(EBotBehaviorType NewType)
{
	BotType = NewType;
...
UAudioComponent* ADroneBot::PlayCharacterSound(USoundCue* CueToPlay)
{
	if (CueToPlay)
	{
...
void ADroneBot::PlayHit(float DamageTaken, struct FDamageEvent const& DamageEvent, APawn* PawnInstigator, AActor* DamageCauser, bool bKilled)
{
...
void ADroneBot::SimulateMeleeStrike_Implementation()
{
...
void ADroneBot::OnMeleeCompBeginOverlap(class AActor* OtherActor, class UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult & SweepResult)
{
	...
	GetWorldTimerManager().SetTimer(TimerHandle_MeleeAttack, this, &ADroneBot::OnRetriggerMeleeStrike, MeleeStrikeCooldown, true);
}


void ADroneBot::OnRetriggerMeleeStrike()
{
	/* Apply damage to a single random pawn in range. */
	TArray<AActor*> Overlaps;
	MeleeCollisionComp->GetOverlappingActors(Overlaps, AAversionInterceptorCharacter::StaticClass());
	for (int32 i = 0; i < Overlaps.Num(); i++)
	{
		AAversionInterceptorCharacter* OverlappingPawn = Cast<AAversionInterceptorCharacter>(Overlaps[i]);
		if (OverlappingPawn)
		{
...
bool ADroneBot::IsSprinting() const
{
...
void ADroneBot::BroadcastUpdateAudioLoop_Implementation(bool bNewSensedTarget)
{
